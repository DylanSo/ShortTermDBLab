<template>
  <table-field v-bind:data_list="data_list">
    ...
    <template v-slot:data_table>
      ...
      <el-table-column label="部门号" prop="syc_dno13" sortable></el-table-column>
      <el-table-column label="部门名" prop="syc_dname13" sortable></el-table-column>
  ...
  methods: {
    ...
    getStudentList() {
      const requestLatest = this.$axios.create({
        baseURL: global.backend.getDepartmentsApi,
        timeout: 1000,
      });
      requestLatest
        .get(global.backend.getDepartmentsApi)
        .then((res) => {
