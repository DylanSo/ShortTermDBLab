<template>
  <table-field v-bind:data_list="data_list">
    ...
    <template v-slot:bread_crumb>
      ...
      <el-breadcrumb-item>学生选课情况</el-breadcrumb-item>
    </template>
    <!-- 插入数据面板-->
    <template v-slot:data_table>
      <el-table-column label="序号" type="index"></el-table-column>
      <el-table-column label="学生姓名" prop="syc_sno13" sortable></el-table-column>
      <el-table-column label="授课号" prop="syc_teno13" sortable></el-table-column>
      <el-table-column label="成绩" prop="syc_grade13" sortable></el-table-column>
      <el-table-column label="是否重修" prop="syc_ifRevamp13"></el-table-column>
  ...
  methods: {
    ...
    getStudentList() {
      const requestLatest = this.$axios.create({
        baseURL: global.backend.getChooseCourseApi,
        timeout: 1000,
      });
      requestLatest
        .get(global.backend.getChooseCourseApi)
        .then((res) => {
